...
final class SidebarThreadGroupingTests: XCTestCase {
    ...
    func testMetricStagingCollectsUniqueVisibleThreadIDsFromGroups() {
        let threads = [
            makeThread(id: "thread-a", updatedAt: .now, cwd: "/Users/me/work/app"),
        ...
        let groups = [
            ...
            SidebarThreadGroup(
                id: "project:/Users/me/work/site",
                label: "site",
                kind: .project,
                sortDate: .now,
                projectPath: "/Users/me/work/site",
                threads: [threads[1]]
            ),
        ]

        let ids = SidebarThreadMetricStaging.visibleThreadIDs(from: groups)

        XCTAssertEqual(ids, Set(["thread-a", "thread-b"]))
    }

    func testMetricStagingPartitionsVisibleIDsBeforeDeferredIDs() {
        let threads = [
            makeThread(id: "thread-a", updatedAt: .now, cwd: "/Users/me/work/app"),
            makeThread(id: "thread-b", updatedAt: .now, cwd: "/Users/me/work/app"),
            makeThread(id: "thread-c", updatedAt: .now, cwd: "/Users/me/work/app"),
        ]
        let visibleIDs = Set(["thread-b", "thread-c"])

        let partition = SidebarThreadMetricStaging.partitionThreadIDs(
            from: threads,
            visibleThreadIDs: visibleIDs
        )

        XCTAssertEqual(partition.visible, ["thread-b", "thread-c"])
        XCTAssertEqual(partition.deferred, ["thread-a"])
    }

    @MainActor
    func testGroupingRebuildDebouncerExecutesActionAfterDelay() async {
        let debouncer = SidebarGroupingRebuildDebouncer()
        let expectation = XCTestExpectation(description: "debounced action runs")
        var runCount = 0

        debouncer.schedule(delayNanoseconds: 20_000_000) {
            runCount += 1
            expectation.fulfill()
        }
    ...
    @MainActor
    func testGroupingRebuildDebouncerCoalescesRapidRequests() async {
        let debouncer = SidebarGroupingRebuildDebouncer()
        let expectation = XCTestExpectation(description: "latest action runs")
        var firstRan = false
        var secondRan = false

        debouncer.schedule(delayNanoseconds: 80_000_000) {
            firstRan = true
        }
        debouncer.schedule(delayNanoseconds: 20_000_000) {
            secondRan = true
    ...
    @MainActor
    func testGroupingRebuildDebouncerCancelPreventsPendingAction() async {
        let debouncer = SidebarGroupingRebuildDebouncer()
        var didRun = false

        debouncer.schedule(delayNanoseconds: 60_000_000) {
            didRun = true
        }
        debouncer.cancel()
